...
export const channelFormSchema = z
  .object({
    ...
    passthrough_cost_path: z.string().optional(), // Passthrough billing: gjson path to upstream cost
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  // Passthrough billing cost path (stored regardless of channel type; only
  // consulted for models configured with passthrough billing mode). Empty
  // string removes the override so the backend default (usage.cost) applies.
  const passthroughCostPath = String(formData.passthrough_cost_path || '').trim()
